    ...
    for (uint32_t hl = 0; len >= sizeof(ip6_ext); SHIFT_DATA_COM(hl)) {
        ...
        switch (next_hdr) {
            ...
            case IPPROTO_FRAGMENT: {
                ...
                hl = sizeof(ip6_frag);
                if (len < hl) return false;
                const ip6_frag* fr_hdr = reinterpret_cast<const ip6_frag*>(data + parsed_bytes); //Указатель на заголовок фрагмента
