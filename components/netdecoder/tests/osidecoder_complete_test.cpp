...
protocols::Decoder decoder;

class OsiDecoderTest : public ::testing::Test
{
public:
    OsiDecoderTest()  {}
...
constexpr std::array<uint8_t, 14> ethHdr{0xcc, 0x03, 0x04, 0xdc, 0x00, 0x10, 0xcc, 0x04, 0x04, 0xdc, 0x00, 0x10, 0x88, 0x47};

TEST_F(OsiDecoderTest, decoder_treat_eth_nullptr_test)
{
    ...
    auto [status, packet] = decoder.treatEth(0, 0, data, size);

    ASSERT_FALSE(status);
    ASSERT_EQ(packet.eth, nullptr);  
    ASSERT_EQ(size, 0);
};

TEST_F(OsiDecoderTest, decoder_treat_vlan_nullptr_test)
{
    auto& decoder = this->getDecoder();
    ...
    auto [status, packet] = decoder.treatVlan(0, 0, data, size);

    ASSERT_FALSE(status);
    ASSERT_EQ(packet.vlan_cnt, 0);
    std::for_each(std::begin(packet.vlans), std::end(packet.vlans), [](auto p) { ASSERT_EQ(p, nullptr); });
    ASSERT_EQ(packet.vlan_cnt, 0);
    ASSERT_EQ(size, 0);
};

TEST_F(OsiDecoderTest, decoder_treat_pppoe_nullptr_test)
{
    auto& decoder = this->getDecoder();
    ...
    auto [status, packet] = decoder.treatPPPoE(0, 0, data, size);

    ASSERT_FALSE(status);
    ASSERT_EQ(packet.pppoe, nullptr);
    ASSERT_EQ(size, 0);
};

TEST_F(OsiDecoderTest, decoder_treat_mpls_nullptr_test)
{
    auto& decoder = this->getDecoder();
    ...
    auto [status, packet] = decoder.treatMpls(0, 0, data, size);

    ASSERT_FALSE(status);
    std::for_each(std::begin(packet.mpls), std::end(packet.mpls), [](auto p) { ASSERT_EQ(p, nullptr); });
    ASSERT_EQ(packet.mpls_cnt, 0);
    ASSERT_EQ(size, 0);
};

TEST_F(OsiDecoderTest, decoder_treat_ip_nullptr_test)
{
    auto& decoder = this->getDecoder();
    ...
    auto [status, packet] = decoder.treatIp(0, 0, data, size);

    ASSERT_FALSE(status);
    ASSERT_EQ(packet.iph, nullptr);
    ASSERT_EQ(packet.ip6h, nullptr);
    ASSERT_EQ(size, 0);
};

TEST_F(OsiDecoderTest, decoder_treat_udp_nullptr_test)
{
    auto& decoder = this->getDecoder();
    ...
    auto [status, packet] = decoder.treatUdp(0, 0, data, size);

    ASSERT_FALSE(status);
    ASSERT_EQ(packet.udph, nullptr);
    ASSERT_EQ(size, 0);
};

TEST_F(OsiDecoderTest, decoder_treat_tcp_nullptr_test)
{
    auto& decoder = this->getDecoder();
    ...
    auto [status, packet] = decoder.treatTcp(0, 0, data, size);

    ASSERT_FALSE(status);
    ASSERT_EQ(packet.tcph, nullptr);
    ASSERT_EQ(size, 0);
};

TEST_F(OsiDecoderTest, decoder_treat_gtp_nullptr_test)
{
    auto& decoder = this->getDecoder();
    ...
    auto [status, packet] = decoder.treatGtp(0, 0, data, size);

    ASSERT_FALSE(status);
    ASSERT_EQ(packet.gtph, nullptr);
    ASSERT_EQ(size, 0);
};
...
TEST_F(OsiDecoderTest, decoder_treat_eth_test)
{
    auto& decoder = this->getDecoder();
    ...
    auto [status, packet] = decoder.treatEth(1, 2, data, size);

    ASSERT_TRUE(status);

    struct ether_header* tEth {(struct ether_header*)ethHdr.data()};

    ASSERT_EQ(packet.eth->ether_dhost, tEth->ether_dhost);
    ASSERT_EQ(packet.eth->ether_shost, tEth->ether_shost);
    ASSERT_EQ(htobe16(packet.eth->ether_type), 0x8847);
...
TEST_F(OsiDecoderTest, decoder_treat_vlan_test)
{
    auto& decoder = this->getDecoder();
    ...
    auto [status, packet] = decoder.treatVlan(1, 2, data, size);

    ASSERT_TRUE(status);

    ASSERT_EQ(packet.tm, 1);
    ASSERT_EQ(packet.tm_ns, 2);
    ASSERT_EQ(packet.vlan_cnt, 2);
    std::for_each(std::begin(packet.vlans), std::end(packet.vlans), [&](auto p) {
        if (p) vlansCntr++;
    });
...
TEST_F(OsiDecoderTest, decoder_treat_pppoe_test)
{
    auto& decoder = this->getDecoder();
    ...
    auto [status, packet] = decoder.treatPPPoE(1, 2, data, size);

    ASSERT_TRUE(status);

    ASSERT_EQ(packet.tm, 1);
    ASSERT_EQ(packet.tm_ns, 2);
    ASSERT_FALSE(packet.pppoe == nullptr);
    ASSERT_EQ(packet.pppoe->code, 0);
    ASSERT_EQ(htobe16(packet.pppoe->payloadLength), 1402);
    ASSERT_EQ(htobe16(packet.pppoe->sessionId), 0x1b26);
    ASSERT_EQ(packet.pppoe->type, 1);
    ASSERT_EQ(packet.pppoe->version, 1);
};
...
TEST_F(OsiDecoderTest, decoder_treat_mpls_test)
{
    auto& decoder = this->getDecoder();
    ...
    auto [status, packet] = decoder.treatMpls(1, 2, data, size);
...
TEST_F(OsiDecoderTest, decoder_treat_ip4_test)
{
    auto& decoder = this->getDecoder();
    ...
    auto [status, packet] = decoder.treatIp(1, 2, data, size);
    ...
    ASSERT_EQ(packet.tm, 1);
    ASSERT_EQ(packet.tm_ns, 2);
    ASSERT_FALSE(packet.iph == nullptr);
    ASSERT_EQ(packet.iph->daddr, tIpH->daddr);
    ASSERT_EQ(packet.iph->saddr, tIpH->saddr);
    ASSERT_EQ(packet.iph->check, tIpH->check);
    ASSERT_EQ(packet.iph->frag_off, tIpH->frag_off);
    ASSERT_EQ(packet.iph->id, tIpH->id);
    ASSERT_EQ(packet.iph->ihl, tIpH->ihl);
    ASSERT_EQ(packet.iph->protocol, tIpH->protocol);
    ASSERT_EQ(packet.iph->tos, tIpH->tos);
    ASSERT_EQ(packet.iph->tot_len, tIpH->tot_len);
    ASSERT_EQ(packet.iph->ttl, tIpH->ttl);
 ...
 */
constexpr std::array<uint8_t, 124> ip6Hdr{0x60, 0x00, 0x00, 0x00, 0x00, 0x54, 0x11, 0xff, 0x26, 0x07, 0xf2, 0xc0, 0xf0, 0x0f, 0xb0, 0x01,
                                         0x00, 0x00, 0x00, 0x00, 0xfa, 0xce, 0xb0, 0x0c, 0x20, 0x01, 0x04, 0xf8, 0x00, 0x03, 0x00, 0x0d,
                                         0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x61, 0x10, 0xf6, 0x10, 0xf6, 0x00, 0x54, 0x8f, 0x5f
                                         //Locator/ID Separation Protocol
                                         ,0x38,0x00,0x01,0x01,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x01,0x00,0x14
                                         ,0xc4,0x3f,0x24,0xc6,0xe7,0x06,0xf9,0xa5,0xbc,0xd0,0x92,0x87,0xf4,0x16,0xb5,0x3e
                                         ,0x43,0x0d,0x58,0xb3,0x00,0x00,0x00,0x05,0x01,0x1c,0x10,0x00,0x00,0x00,0x00,0x01
                                         ,0x99,0x10,0x1f,0x50,0x01,0x64,0xff,0x00,0x00,0x05,0x00,0x02,0x26,0x07,0xf2,0xc0
                                         ,0xf0,0x0f,0xb0,0x01,0x00,0x00,0x00,0x00,0xfa,0xce,0xb0,0x0c
};
TEST_F(OsiDecoderTest, decoder_treat_ip6_test)
{
    ...
    ASSERT_EQ(packet.tm, 1);
    ASSERT_EQ(packet.tm_ns, 2);
    ASSERT_FALSE(packet.ip6h == nullptr);
    ASSERT_TRUE(std::equal(std::begin(*(Ip6AddrP)&packet.ip6h->ip6_src), std::end(*(Ip6AddrP)&packet.ip6h->ip6_src),
                           std::begin(*(Ip6AddrP)&tIp6H->ip6_src)));
    ASSERT_TRUE(std::equal(std::begin(*(Ip6AddrP)&packet.ip6h->ip6_dst), std::end(*(Ip6AddrP)&packet.ip6h->ip6_dst),
                           std::begin(*(Ip6AddrP)&tIp6H->ip6_dst)));
    ASSERT_EQ(packet.ip6h->ip6_ctlun.ip6_un2_vfc, tIp6H->ip6_ctlun.ip6_un2_vfc);
    ASSERT_EQ(packet.ip6h->ip6_ctlun.ip6_un1.ip6_un1_flow, tIp6H->ip6_ctlun.ip6_un1.ip6_un1_flow);
    ASSERT_EQ(packet.ip6h->ip6_ctlun.ip6_un1.ip6_un1_hlim, tIp6H->ip6_ctlun.ip6_un1.ip6_un1_hlim);
    ASSERT_EQ(packet.ip6h->ip6_ctlun.ip6_un1.ip6_un1_nxt, tIp6H->ip6_ctlun.ip6_un1.ip6_un1_nxt);
    ASSERT_EQ(packet.ip6h->ip6_ctlun.ip6_un1.ip6_un1_plen, tIp6H->ip6_ctlun.ip6_un1.ip6_un1_plen);
    ASSERT_EQ(size, 84);
};
...
TEST_F(OsiDecoderTest, decoder_treat_udp_test)
{
    auto& decoder = this->getDecoder();
    ...
    auto [status, packet] = decoder.treatUdp(1, 2, data, size);

    ASSERT_TRUE(status);
    ASSERT_EQ(packet.tm, 1);
    ASSERT_EQ(packet.tm_ns, 2);
    ASSERT_FALSE(packet.udph == nullptr);
    ASSERT_EQ(htobe16(packet.udph->check),0xc643);
    ASSERT_EQ(htobe16(packet.udph->dest),18569);
    ASSERT_EQ(htobe16(packet.udph->source),35024);
    ASSERT_EQ(htobe16(packet.udph->len),28);
 ...
 */
constexpr std::array<uint8_t, 32> tcpHdr{0x01,0xbb,0xc8,0x32,0xcf,0x0f,0x6a,0xca,0x26,0xaa,0xee,0xce,0x80,0x10,0x0a,0xd5,0x40,0x0b,0x00,0x00,0x01,0x01,0x08,0x0a,0xc8,0xb7,0xbd,0x88,0x03,0x59,0xed,0xa8};
TEST_F(OsiDecoderTest, decoder_treat_tcp_test)
{
    auto& decoder = this->getDecoder();
    ...
    auto [status, packet] = decoder.treatTcp(1, 2, data, size);


    auto * tcp = packet.tcph;
...
TEST_F(OsiDecoderTest, decoder_treat_gtp_test)
{
    auto& decoder = this->getDecoder();
    ...
    auto [status, packet] = decoder.treatGtp(1, 2, data, size);

    const GtpHeader * gtp = packet.gtph;
    ...
    ASSERT_FALSE(packet.is_gtpv1_hdr_ext());
    ASSERT_EQ(size, 94);
};
 ...
 */
constexpr std::array<uint8_t, 28> sctpHdr{0x40, 0x00, 0x0b, 0x80, 0x00, 0x01, 0x6f, 0x0a, 0x6d, 0xb0, 0x18, 0x82, 0x00, 0x03,
                                          0x00, 0x5b, 0x28, 0x02, 0x43, 0x45, 0x00, 0x00, 0xa0, 0xbd, 0x00, 0x00, 0x00, 0x07};

TEST_F(OsiDecoderTest, decoder_treat_sctp_test)
{
    auto& decoder = this->getDecoder();
    ...
    auto [status, packet] = decoder.treatSctp(1, 2, data, size);

    const sctphdr * sctp = packet.sctph;
